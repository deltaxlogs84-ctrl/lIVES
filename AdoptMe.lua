...
local function UpdateNamesTradeApp()
	...
	if getgenv().ForcedPartnerName then
		pcall(function()
			local v200 = get_platform() == "mobile" and 140 or 220

			if SavedApp.negotiation_partner_name_label then
				SavedApp.negotiation_partner_name_label.Text = getgenv().ForcedPartnerName
				SavedApp.negotiation_partner_name_label.Size = UDim2.new(0, v200, 1, 0)
				SavedApp.negotiation_partner_name_label.Size = UDim2.new(0, math.min(v200, SavedApp.negotiation_partner_name_label.TextBounds.X), 1, 0)
			end
...
MiscTab:Button({
	...
	Callback = function()
		...
		if Selected then
			...
			--[[
			wait(0.1)
			btn = game.CoreGui.BlockingModalScreen.BlockingModalContainer.BlockingModalContainerWrapper.BlockingModal.AlertModal.AlertContents.Footer.Buttons["3"]
